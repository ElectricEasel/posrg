...
$i		= 0;
$Itemid = JRequest::getVar('Itemid', 163);
?>
<form action="index.php" method='get' name='adminForm' id='adminForm' class='gm-form'>
	...
	<thead>
		<tr>
			...
			<th>
				...
				<?php echo JHTML::_('grid.sort', JText::_('GM_PRODUCT_MANAGER_DES'), 'product_des', $this->lists['order_Dir'], $this->lists['order'] ); ?>
...
if ($count) :
	...
	if(JFile::exists(JPATH_COMPONENT . DS . 'assets' . DS . 'upload' . DS . $item->image )){
	...
	$task 		= $item->published  ? 'block' 					: 'unblock';
...
?>
		<tr class="<?php echo "row$k"; ?>">
			...
			<td>
				<a href="<?php echo JRoute::_('index.php?option=com_product&id=' . $item->id. "&task=" . $task . '&' . JUtility::getToken() . '=1') ?>" >
			...
			<td>
				...
				<a href="<?php echo JRoute::_('index.php?option=com_product&task=delete&id=' . $item->id . '&' . JUtility::getToken() . '=1' ) ?>"><span>Delete</span></a>
			</td>
		</tr>
		<?php $k = 1 - $k; ?>
<?php
