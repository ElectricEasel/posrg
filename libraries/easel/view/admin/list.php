...
class EEViewAdminList extends EEViewBase
{
	...
	public function display($tpl = null)
	{
        ...
        if (method_exists($this->helperName, 'addSubmenu'))
        {
    		call_user_func(array($this->helperName, 'addSubmenu'), $view);
        }
	...
	protected function addToolbar()
	{
		...
		{
			if (isset($this->items[0]->state) || isset($this->items[0]->published))
			{
		...
		if (isset($this->items[0]->state) || isset($this->items[0]->published))
		{
			if (($state->get('filter.state') == -2 || $state->get('filter.published') == -2) && $canDo->get('core.delete'))
			{
