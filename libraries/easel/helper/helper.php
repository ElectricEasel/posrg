...
abstract class EEHelper
{
	...
	{
		$defaultPhone = EEComponentHelper::getParam('com_rv', 'default_phone', '8775702267');
		$phone = JFactory::getApplication()->getUserState('advert_phone', $defaultPhone);
        ...
        if ($lang == 'french')
        {
            switch ($state)
            {
                case EE_TABLE_ERROR:
                    ...
                    break;
                case EE_ERROR_MODEL:
                    $msg = 'Could not find form model.';
                    break;
                case EE_FORM_VALIDATION_FAILED:
                    $msg = 'Votre forme n\'a pas validé. S\'il vous plaît remplir tous les champs obligatoires.';
                    break;
                case EE_ERROR_UNRECOVERABLE:
                    $msg = 'An unknown error occurred. Please <a href="javascript:document.location.reload(true)">refresh the page</a> and try again. (french)';
                    break;
                default:
                    $msg = 'Unknown Error';
        ...
        else
        {
            switch ($state)
            {
                ...
                case EE_ERROR_MODEL:
                    ...
                    break;
                case EE_FORM_VALIDATION_FAILED:
                    $msg = 'We were not able to complete your request. Please fill out all required fields.';
                    break;
                case EE_ERROR_UNRECOVERABLE:
                    $msg = 'An unknown error occurred. Please <a href="javascript:document.location.reload(true)">refresh the page</a> and try again.';
                    break;
                default:
                    $msg = 'Unknown Error';
                    break;
	...
	public static function formatDataForEmail(array $data, JForm $form)
	{
		...
		// Remove the anti-spam fields from the submitted values.
		unset($data['antispam']);
     ...
     * @return  void
     */
    public static function updateSessionData(array $data, $name = 'general', $group = 'formdata')
	{
		$session = JFactory::getSession();
		$current = $session->get($name, array(), $group);
		...
		$session->set($name, array_merge($current, $data), $group);
	}
     ...
     */
    public static function getSessionData($name = 'general', $group = 'formdata')
	{
		$current = JFactory::getSession()->get($name, array(), $group);
    ...
    public static function detectLangForView($viewName = null)
    {
        if ($viewName === null)
        {
        ...
        if (strpos($viewName, 'french') !== false)
        {
            return 'french';
        }
